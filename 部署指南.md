...
复制default_config.yaml 为 config.yaml 并修改对应配置。
...
附：
如果在linux环境下想使用go-judge，可以先通过deploy/go-judge目录下的Dockerfile构建镜像并运行

```bash
docker build -t patreon-judge ./deploy/go-judge
docker run -d --privileged --rm -p 5050:5050 --name go-judge patreon-judge
```

在window环境下启动minio命令
minio
```bash
.\minio.exe server D:\minio\data --address ":9090" --console-address ":9001"
```
